# Comparing `tmp/RUTTS-0.0.7.tar.gz` & `tmp/RUTTS-0.0.8.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "RUTTS-0.0.7.tar", last modified: Sat Jul 29 01:01:13 2023, max compression
+gzip compressed data, was "RUTTS-0.0.8.tar", last modified: Tue Aug  1 19:38:00 2023, max compression
```

## Comparing `RUTTS-0.0.7.tar` & `RUTTS-0.0.8.tar`

### file list

```diff
@@ -1,14 +1,14 @@
-drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-07-29 01:01:13.729437 RUTTS-0.0.7/
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)     1057 2023-07-27 22:12:45.000000 RUTTS-0.0.7/LICENSE.txt
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      561 2023-07-29 01:01:13.729216 RUTTS-0.0.7/PKG-INFO
-drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-07-29 01:01:13.726362 RUTTS-0.0.7/RUTTS/
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       27 2023-07-27 21:39:42.000000 RUTTS-0.0.7/RUTTS/__init__.py
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)     3045 2023-07-29 00:59:38.000000 RUTTS-0.0.7/RUTTS/infer_onnx.py
-drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-07-29 01:01:13.728789 RUTTS-0.0.7/RUTTS.egg-info/
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      561 2023-07-29 01:01:13.000000 RUTTS-0.0.7/RUTTS.egg-info/PKG-INFO
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      202 2023-07-29 01:01:13.000000 RUTTS-0.0.7/RUTTS.egg-info/SOURCES.txt
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        1 2023-07-29 01:01:13.000000 RUTTS-0.0.7/RUTTS.egg-info/dependency_links.txt
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       66 2023-07-29 01:01:13.000000 RUTTS-0.0.7/RUTTS.egg-info/requires.txt
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        6 2023-07-29 01:01:13.000000 RUTTS-0.0.7/RUTTS.egg-info/top_level.txt
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       38 2023-07-29 01:01:13.729510 RUTTS-0.0.7/setup.cfg
--rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      731 2023-07-29 01:00:24.000000 RUTTS-0.0.7/setup.py
+drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-08-01 19:38:00.686061 RUTTS-0.0.8/
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)     1057 2023-07-27 22:12:45.000000 RUTTS-0.0.8/LICENSE.txt
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      561 2023-08-01 19:38:00.685835 RUTTS-0.0.8/PKG-INFO
+drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-08-01 19:38:00.682841 RUTTS-0.0.8/RUTTS/
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       27 2023-07-27 21:39:42.000000 RUTTS-0.0.8/RUTTS/__init__.py
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)     3134 2023-08-01 19:32:45.000000 RUTTS-0.0.8/RUTTS/infer_onnx.py
+drwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        0 2023-08-01 19:38:00.685331 RUTTS-0.0.8/RUTTS.egg-info/
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      561 2023-08-01 19:38:00.000000 RUTTS-0.0.8/RUTTS.egg-info/PKG-INFO
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      202 2023-08-01 19:38:00.000000 RUTTS-0.0.8/RUTTS.egg-info/SOURCES.txt
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        1 2023-08-01 19:38:00.000000 RUTTS-0.0.8/RUTTS.egg-info/dependency_links.txt
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       66 2023-08-01 19:38:00.000000 RUTTS-0.0.8/RUTTS.egg-info/requires.txt
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)        6 2023-08-01 19:38:00.000000 RUTTS-0.0.8/RUTTS.egg-info/top_level.txt
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)       38 2023-08-01 19:38:00.686142 RUTTS-0.0.8/setup.cfg
+-rwxrwxrwx   0 teraspace  (1000) teraspace  (1000)      731 2023-08-01 19:37:17.000000 RUTTS-0.0.8/setup.py
```

### Comparing `RUTTS-0.0.7/LICENSE.txt` & `RUTTS-0.0.8/LICENSE.txt`

 * *Files identical despite different names*

### Comparing `RUTTS-0.0.7/PKG-INFO` & `RUTTS-0.0.8/PKG-INFO`

 * *Files 21% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: RUTTS
-Version: 0.0.7
+Version: 0.0.8
 Summary: russian text to speech
 Home-page: https://github.com/Tera2Space/RUTTS
 Author: Tera Space
 Author-email: tera2space@gmail.com
 License: MIT
 Keywords: tts
 Classifier: Development Status :: 5 - Production/Stable
```

### Comparing `RUTTS-0.0.7/RUTTS/infer_onnx.py` & `RUTTS-0.0.8/RUTTS/infer_onnx.py`

 * *Files 8% similar despite different names*

```diff
@@ -11,17 +11,19 @@
     def __init__(self, model_name: str, save_path: str = "./model") -> None:
         if not os.path.exists(save_path):
             os.mkdir(save_path)
         
         model_dir = os.path.join(save_path, model_name)
         
         if not os.path.exists(model_dir):
-            snapshot_download(repo_id=model_name, allow_patterns=["*.txt", "*.onnx"], 
-                              local_dir=model_dir
-                              )
+            snapshot_download(repo_id=model_name, 
+                              allow_patterns=["*.txt", "*.onnx"], 
+                              local_dir=model_dir,
+                              local_dir_use_symlinks=False
+                            )
         
         sess_options = onnxruntime.SessionOptions()
         self.model = onnxruntime.InferenceSession(os.path.join(model_dir, "exported/model.onnx"), sess_options=sess_options)
         
         with open(os.path.join(model_dir, "exported/vocab.txt"), "r", encoding="utf-8") as vocab_file:
             self.symbols = vocab_file.read().split("\n")
             self.symbols = list(map(chr, list(map(int, self.symbols))))
```

### Comparing `RUTTS-0.0.7/RUTTS.egg-info/PKG-INFO` & `RUTTS-0.0.8/RUTTS.egg-info/PKG-INFO`

 * *Files 21% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: RUTTS
-Version: 0.0.7
+Version: 0.0.8
 Summary: russian text to speech
 Home-page: https://github.com/Tera2Space/RUTTS
 Author: Tera Space
 Author-email: tera2space@gmail.com
 License: MIT
 Keywords: tts
 Classifier: Development Status :: 5 - Production/Stable
```

### Comparing `RUTTS-0.0.7/setup.py` & `RUTTS-0.0.8/setup.py`

 * *Files 1% similar despite different names*

```diff
@@ -8,15 +8,15 @@
   'Operating System :: MacOS',
   'License :: OSI Approved :: MIT License',
   'Programming Language :: Python :: 3'
 ]
  
 setup(
   name='RUTTS',
-  version='0.0.7',
+  version='0.0.8',
   description='russian text to speech',
   url='https://github.com/Tera2Space/RUTTS',  
   author='Tera Space',
   author_email='tera2space@gmail.com',
   license='MIT', 
   classifiers=classifiers,
   keywords='tts',
```

